...
## 1. Về cấu trúc thư mục tiêu chuẩn
...
- Bổ sung comment nếu nghĩ rằng chính mình cũng sẽ quên chức năng của đoạn code đó.
...
- Không viết `static class` nếu không có lý do.
- Không viết nhiều logic vào class properties. Đổi sang method để làm chuyện đó.
- Không sử dụng C# event. Sử dụng Pattern Observer.
...
- Tất cả dòng khai báo sử dụng ServiceLocator phải được khai báo trong hàm Awake()
- Class C# thường (không phải MonoBehaviour): không được sử dụng trực tiếp ServiceLocator, phải truyền object cần sử dụng từ constructor.
